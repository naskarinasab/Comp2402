...
BinarySearchTree<CountdownTree.Node<T>, T> implements SSet<T> {
	...
	void rebalance(Node<T> u) {
		int b = size(u);
		Node<T> p = u.parent;
		Node<T>[] a = (Node<T>[]) Array.newInstance(Node.class, b);

		packIntoArray(u, a, 0);

		if (p == nil) {
			r = buildBalanced(a, 0, b);
			r.parent = nil;
		} else if (p.right == u) {
			p.right = buildBalanced(a, 0, b);
			p.right.parent = p;
		} else {
			p.left = buildBalanced(a, 0, b);
			p.left.parent = p;
		}
	}
	...
	Node<T> buildBalanced(Node<T>[] a, int i, int b) {
		if (b == 0) return nil;
		int m = b / 2;
		a[i + m].left = buildBalanced(a, i, m);

		if (a[i + m].left != nil) a[i + m].left.parent = a[i + m];
		a[i + m].right = buildBalanced(a, i + m + 1, b - m - 1);

		if (a[i + m].right != nil) a[i + m].right.parent = a[i + m];
